# Overview
The Creation Kit Audit Tool is an adjunct tool that one can run in conjunection with the [Starfield Creation Kit](https://store.steampowered.com/app/2722710/Starfield_Creation_Kit/) to automate the process of reliably creating ARCHLIST packing lists for one's plugins.  This tool solves
...
3. Works with a suitably configured WWise deployment to create platform-specific ARCHLIST packing lists in a single pass.
...
that audit log to generate suitable ARCHLIST packing lists for the Creation Kit.
...
`XBox WEM Path` in the Creation Kit Audit Tool and when the tool generates the platform-specific ARCHLIST packing lists, the PC
ARCHLIST file will pack the WEM files from the standard directory tree and the XBox ARCHLIST file will pack the WEM files from
